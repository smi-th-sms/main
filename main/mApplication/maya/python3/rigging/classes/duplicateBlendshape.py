...
def duplicateTarget(object, blendshape_, newMesh):
            ...
            for name, value in target_:
                try:
                    ...
                    if not pm.objExists(name):
                        target_ = pm.duplicate(newMesh, n='{0}'.format(name), rr=1)
                        '''
                        ...
                        target_ = pm.duplicate(newMesh, n='{0}Dup'.format(name), rr=1)
                    bsAttr.set(0)
...
duplicateTarget(object, blendshape_, newMesh)


'''
sel = cmds.ls(sl=1,fl=1,r=1)

def getBlendshape2(object):
    hlist_ = cmds.listHistory(object[0])
    for h in hlist_:
        if cmds.objectType(h, isa = 'blendShape'):
            return(h)

blendshape_ = getBlendshape2(sel)

bs_ = cmds.aliasAttr(blendshape_,q=1)
bs_.sort()
half = int(len(bs_)/2)
bs_ = bs_[:half]


cmds.listAttr(blendshape_)
'''
