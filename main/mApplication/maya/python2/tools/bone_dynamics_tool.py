...
Usage (Maya Script Editor):
    import importlib
    from python2.tools import bone_dynamics_tool
...
# tools/ -> python2/ -> maya/ -> mApplication/ -> main/ -> utils/
...
class BoneDynamicsTool(QtWidgets.QDialog):
    ...
    def _check_plugin_status(self):
        try:
            ...
            self.plugin_status_label.setText("Plugin: 로드 실패 - {}".format(e))
    ...
    def _add_selected_joints(self):
        ...
        self._log("[INFO] {}개 조인트 추가됨.".format(len(sel)))
    ...
    def _add_selected_colliders(self):
        ...
        self._log("[INFO] {}개 콜라이더 오브젝트 추가됨.".format(len(sel)))
    ...
    def _expand_joint_chain(self, joints):
        ...
        for next_joint in joints[1:]:
            ...
            if not path:
                self._log(
                    "[WARNING] {} -> {} 사이의 경로를 찾을 수 없습니다. 그대로 이어붙입니다.".format(
                        current, next_joint
                    )
                )
                expanded.append(next_joint)
    ...
    def _undo_last(self):
        ...
        try:
            ...
            self._log("[ERROR] Undo 실패: {}".format(e))
        finally:
            self.undo_btn.setEnabled(False)
    ...
    def _execute(self):
        ...
        self._log("=" * 52)
        self._log("[START] Bone Dynamics ({}개 세그먼트)".format(len(joints) - 1))
        self._log("  Joints   : {}".format(' -> '.join(joints)))
        self._log("=" * 52)
        ...
        for bone, end, target_bone in zip(joints[:-1], joints[1:], target_joints[:-1]):
            ...
            if node:
                ...
                self._log("  -> {} : {}".format(bone, node))
                created += 1

        self._log("=" * 52)
        self._log("[DONE] {}/{}개 노드 생성 완료. (Set: {})".format(created, len(joints) - 1, SET_NAME))
        self._log("=" * 52)
