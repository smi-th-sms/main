# -*- coding: utf-8 -*-
"""
MH Tools Integrated - Maya Tools Suite (Python 2)
Maya 작업을 위한 통합 툴

사용법 1 (권장 - 런처 사용):
    from python2 import launch_mh_tools
    launch_mh_tools.launch()

    # 또는
    import python2
    python2.launch_tools()

사용법 2 (직접 import):
    from python2.tools import mh_tools_integrated
    reload(mh_tools_integrated)
...
class MHToolsIntegrated(object):
    """통합 Maya 툴"""
    ...
    def fbx_import(self, *args):
        ...
        if not os.path.exists(file_path):
            cmds.confirmDialog(title="오류", message=u"파일을 찾을 수 없습니다:\n{}".format(file_path), button=["확인"])
            return
        ...
        try:
            ...
            print(u"FBX 파일 임포트 완료: {}".format(file_path))
            cmds.confirmDialog(title="완료", message=u"FBX 파일 임포트 완료!\n\n{}".format(os.path.basename(file_path)), button=["확인"])
        except Exception as e:
            cmds.confirmDialog(title="오류", message=u"FBX 임포트 중 오류 발생: {}".format(str(e)), button=["확인"])
    ...
    def delete_selected_namespaces(self, *args):
        ...
        confirm = cmds.confirmDialog(title="확인", 
                                     message=u"선택한 {}개의 네임스페이스를 삭제하시겠습니까?\n\n{}".format(len(selected), ', '.join(selected)), 
        ...
        if confirm == "삭제":
            ...
            for ns in selected:
                try:
                    ...
                    pass
            self.refresh_namespaces()
            cmds.confirmDialog(title="완료", message=u"삭제 완료: {}개".format(success), button=["확인"])
    ...
    def delete_all_namespaces(self, *args):
        ...
        confirm = cmds.confirmDialog(title="경고", 
                                     message=u"모든 네임스페이스({}개)를 삭제하시겠습니까?".format(len(namespaces)), 
                                     button=["삭제", "취소"], defaultButton="취소", cancelButton="취소")
        ...
        if confirm == "삭제":
            ...
            for ns in namespaces:
                try:
                    ...
                    pass
            self.refresh_namespaces()
            cmds.confirmDialog(title="완료", message=u"전체 삭제 완료: {}개".format(success), button=["확인"])
        ...
        if layers:
            for layer_type, layer_name in layers:
                if layer_type == 'display':
                    display_text = u"[Display] {}".format(layer_name)
                else:  # anim
                    display_text = u"[Anim] {}".format(layer_name)
    ...
    def delete_selected_layers(self, *args):
        ...
        confirm = cmds.confirmDialog(title="확인", 
                                     message=u"선택한 {}개의 레이어를 삭제하시겠습니까?\n\n{}".format(len(layer_names), ', '.join(layer_names)), 
                                     button=["삭제", "취소"], defaultButton="취소", cancelButton="취소")
        ...
        if confirm == "삭제":
            ...
            for layer in layer_names:
                try:
                    ...
                    failed.append(u"{}: {}".format(layer, str(e)))
            
            self.refresh_layers()
            
            if failed:
                cmds.confirmDialog(title="완료", 
                                 message=u"삭제 완료: {}개\n실패: {}개".format(success, len(failed)), 
                                 button=["확인"])
            else:
                cmds.confirmDialog(title="완료", message=u"삭제 완료: {}개".format(success), button=["확인"])
    ...
    def delete_all_layers(self, *args):
        ...
        confirm = cmds.confirmDialog(title="경고", 
                                     message=u"모든 레이어를 삭제하시겠습니까?\n\nDisplay: {}개\nAnimation: {}개\n총: {}개".format(display_count, anim_count, len(layers)), 
                                     button=["삭제", "취소"], defaultButton="취소", cancelButton="취소")
        ...
        if confirm == "삭제":
            ...
            for layer_type, layer_name in layers:
                try:
                    ...
                    failed.append(u"{}: {}".format(layer_name, str(e)))
    ...
    def execute_cleanup_task1(self):
        ...
        try:
            ...
            if cmds.objExists('joints_grp'):
                ...
                if children:
                    self.cleanup_log(u"  ✓ joints_grp에서 {}개 오브젝트 발견".format(len(children)))
                    ...
                    for child in children:
                        try:
                            ...
                            self.cleanup_log(u"  ✓ Unparent: {}".format(short_name))
                        except Exception as e:
                            self.cleanup_log(u"  ✗ Unparent 실패: {} - {}".format(child, str(e)))
                else:
            ...
            if unparented_joints:
                ...
                if root_joints:
                    self.cleanup_log(u"  ✓ 'root'로 시작하는 조인트 트리 발견: {}개".format(len(root_joints)))
                    ...
                    if spine_joint:
                        self.cleanup_log(u"  ✓ spine_03 조인트 발견: {}".format(spine_joint.split('|')[-1]))
                        ...
                        for joint_name in unparented_joints:
                            try:
                                ...
                                if found_joints:
                                    ...
                                    self.cleanup_log(u"  ✓ Parent: {} → {}".format(joint_name, spine_joint.split('|')[-1]))
                                    success_count += 1
                            except Exception as e:
                                self.cleanup_log(u"  ✗ Parent 실패: {} - {}".format(joint_name, str(e)))
                        
                        self.cleanup_log(u"  ✓ 조인트 parent 완료: {}/{}개".format(success_count, len(unparented_joints)))
                    else:
            ...
            if joints_grp_exists and cmds.objExists('joints_grp'):
                try:
                    ...
                    self.cleanup_log(u"  ✗ joints_grp 제거 실패: {}".format(str(e)))
            ...
            if cmds.objExists('geometry_grp'):
                try:
                    ...
                    self.cleanup_log(u"  ✗ geometry_grp 리네임 실패: {}".format(str(e)))
            else:
            ...
            if cmds.objExists('head_grp'):
                ...
                try:
                    ...
                    if asset_name:
                        ...
                        self.cleanup_log(u"  ✓ head_grp → {} 리네임 완료".format(new_name))
                        
                        try:
                            cmds.setAttr(u"{}.useOutlinerColor".format(new_name), 1)
                            cmds.setAttr(u"{}.outlinerColor".format(new_name), 0, 0.849, 0, type='double3')
                            self.cleanup_log(u"  ✓ {} 아웃라이너 색상 설정 완료 (녹색)".format(new_name))
                        except Exception as e:
                            self.cleanup_log(u"  ✗ 아웃라이너 색상 설정 실패: {}".format(str(e)))
                    else:
                        ...
                        if cmds.objExists(parent_name):
                            ...
                            if not children:
                                try:
                                    ...
                                    self.cleanup_log(u"  ✓ 빈 상위 그룹 제거: {}".format(parent_name))
                                except Exception as e:
                                    self.cleanup_log(u"  ✗ 상위 그룹 제거 실패: {}".format(str(e)))
                            else:
                                self.cleanup_log(u"  ℹ 상위 그룹 '{}'이 비어있지 않아 제거하지 않습니다.".format(parent_name))
                    
                except Exception as e:
                    self.cleanup_log(u"  ✗ head_grp 처리 실패: {}".format(str(e)))
            else:
            ...
            self.cleanup_log(u"[작업 1] 오류: {}".format(str(e)))
    ...
    def execute_cleanup_task2(self):
        ...
        try:
            ...
            for grp in grps_to_delete:
                try:
                    cmds.delete(grp)
                    self.cleanup_log(u"  ✓ 삭제: {}".format(grp))
                    deleted_count += 1
                except Exception as e:
                    self.cleanup_log(u"  ✗ 삭제 실패: {} - {}".format(grp, str(e)))
            
            self.cleanup_log(u"[작업 2] 완료! ({}개 그룹 삭제)".format(deleted_count))
            
        except Exception as e:
            self.cleanup_log(u"[작업 2] 오류: {}".format(str(e)))
    ...
    def execute_cleanup_task3(self):
        ...
        try:
            ...
            self.cleanup_log(u"  ✓ 부모 조인트 발견: neck({}개), head({}개)".format(len(neck_joints), len(head_joints)))
            ...
            for parent_joint in parent_joints:
                ...
                if not facial_root_joints:
                    self.cleanup_log(u"  ℹ {}에 FACIAL 루트 조인트가 없습니다.".format(parent_joint))
                    continue
                
                self.cleanup_log(u"  ✓ {}에서 {}개 FACIAL 루트 조인트 발견".format(parent_joint, len(facial_root_joints)))
                ...
                for facial_root in facial_root_joints:
                    try:
                        ...
                        self.cleanup_log(u"  ✓ Constraint: {} → {}".format(short_parent, short_facial))
                        constraint_count += 1
                        ...
                        short_facial = facial_root.split('|')[-1]
                        self.cleanup_log(u"  ✗ Constraint 실패: {} - {}".format(short_facial, str(e)))
            
            self.cleanup_log(u"  ✓ Constraint 적용 완료: {}개".format(constraint_count))
            
            if facial_top_joints:
                facial_top_joints = list(set(facial_top_joints))
                self.cleanup_log(u"  ✓ 최상위 FACIAL 조인트: {}개".format(len(facial_top_joints)))
                
                if cmds.objExists('headjoint_grp'):
                    cmds.delete('headjoint_grp')
                
                headjoint_grp = cmds.group(empty=True, name='headjoint_grp')
                self.cleanup_log(u"  ✓ 그룹 생성: {}".format(headjoint_grp))
                
                for facial_joint in facial_top_joints:
                    try:
                        short_name = facial_joint.split('|')[-1]
                        cmds.parent(facial_joint, headjoint_grp)
                        self.cleanup_log(u"  ✓ Parent: {} → headjoint_grp".format(short_name))
                    except Exception as e:
                        short_name = facial_joint.split('|')[-1]
                        self.cleanup_log(u"  ✗ Parent 실패: {} - {}".format(short_name, str(e)))
                
                if cmds.objExists('headRig_grp'):
                    try:
                        cmds.parent(headjoint_grp, 'headRig_grp')
                        self.cleanup_log(u"  ✓ Parent: headjoint_grp → headRig_grp")
                    except Exception as e:
                        self.cleanup_log(u"  ✗ Parent 실패: {}".format(str(e)))
                else:
            ...
            self.cleanup_log(u"[작업 3] 오류: {}".format(str(e)))
        ...
        try:
            ...
            for lights_grp in lights_grps:
                try:
                    ...
                    self.cleanup_log(u"  ✓ 삭제: {}".format(lights_grp))
                    deleted_count += 1
                except Exception as e:
                    self.cleanup_log(u"  ✗ 삭제 실패: {} - {}".format(lights_grp, str(e)))
            
            self.cleanup_log(u"[작업 4] 완료! ({}개 그룹 삭제)".format(deleted_count))
            
        except Exception as e:
            self.cleanup_log(u"[작업 4] 오류: {}".format(str(e)))
    ...
    def execute_cleanup_task5(self):
        ...
        try:
            ...
            if unknown_plugins:
                ...
                for plugin in unknown_plugins:
                    try:
                        ...
                        self.cleanup_log(u"  ✓ Unknown plugin 제거: {}".format(plugin))
                        removed_count += 1
                    except Exception as e:
                        self.cleanup_log(u"  ✗ Plugin 제거 실패: {} - {}".format(plugin, str(e)))
                
                self.cleanup_log(u"  ✓ Unused plugins 제거 완료 ({}개)".format(removed_count))
            else:
            ...
            self.cleanup_log(u"[작업 5] 오류: {}".format(str(e)))
    ...
    def execute_cleanup_task6(self):
        ...
        try:
            ...
            if created_count > 0:
                self.cleanup_log(u"[작업 6] 완료! ({}개 Sets 생성)".format(created_count))
            else:
                self.cleanup_log("[작업 6] 완료! (모든 Sets가 이미 존재함)")
            
        except Exception as e:
            self.cleanup_log(u"[작업 6] 오류: {}".format(str(e)))
    ...
    def load_skin_objects(self, *args):
        ...
        for i, obj in enumerate(selected):
            cmds.textScrollList(self.skin_object_list, edit=True, append=u"{}. {}".format(i + 1, obj))
        
        self.skin_log(u"\n✓ {}개 오브젝트 로드됨".format(len(selected)))
        
        if mode == 'mode_1toN':
            if len(selected) >= 2:
                self.skin_log(u"  Source: {}".format(selected[0]))
                self.skin_log(u"  Targets: {}개".format(len(selected) - 1))
        elif mode == 'mode_NtoN':
            if len(selected) >= 2 and len(selected) % 2 == 0:
                half = len(selected) // 2
                self.skin_log(u"  Sources: {}개".format(half))
                self.skin_log(u"  Targets: {}개".format(half))
    ...
    def _copy_skin_1toN(self, source, targets):
        ...
        self.skin_log(u"\n[1:N 모드] 스킨 복사 시작")
        self.skin_log(u"Source: {}".format(source))
        
        bind_joints = self._get_bind_joints(source)
        if not bind_joints:
            self.skin_log(u"✗ Source에 skinCluster가 없습니다: {}".format(source))
            return 0
        
        self.skin_log(u"✓ 바인드 조인트: {}개".format(len(bind_joints)))
        ...
        for target in targets:
            try:
                ...
                self.skin_log(u"✓ {}".format(target))
                success_count += 1
                
            except Exception as e:
                self.skin_log(u"✗ {} - {}".format(target, str(e)))
    ...
    def _copy_skin_NtoN(self, sources, targets):
        ...
        self.skin_log(u"\n[N:N 모드] 스킨 복사 시작")
        self.skin_log(u"Sources: {}개".format(len(sources)))
        self.skin_log(u"Targets: {}개".format(len(targets)))
        ...
        for idx, source in enumerate(sources):
            ...
            try:
                ...
                if not src_skin:
                    self.skin_log(u"✗ [{}] {} - skinCluster 없음".format(idx + 1, source))
                    continue
                ...
                if not joints:
                    self.skin_log(u"✗ [{}] {} - 조인트 없음".format(idx + 1, source))
                    continue
                ...
                self.skin_log(u"✓ [{}] {} → {}".format(idx + 1, source, target))
                success_count += 1
                
            except Exception as e:
                self.skin_log(u"✗ [{}] {} → {} - {}".format(idx + 1, source, target, str(e)))
    ...
    def execute_skin_copy(self, *args):
        ...
        try:
            ...
            self.skin_log(u"\n{}".format('=' * 40))
            self.skin_log(u"완료! 성공: {}개".format(success_count))
            self.skin_log(u"{}".format('=' * 40))
            
            cmds.confirmDialog(title="완료", message=u"스킨 복사 완료!\n\n성공: {}개".format(success_count), button=["확인"])
            
        except Exception as e:
            error_msg = u"스킨 복사 중 오류 발생: {}".format(str(e))
            self.skin_log(u"\n✗ {}".format(error_msg))
...
def reload_and_show():
    ...
    if module_name in sys.modules:
        ...
        print(u"모듈 리로드 완료: {}".format(module_name))
    show()
